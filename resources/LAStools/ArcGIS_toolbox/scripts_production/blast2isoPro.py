...
#
# uses blast2iso.exe to contour a folder of LiDAR files
#
...
gp.AddMessage("Starting blast2iso production ...")
...
### create the full path to the blast2iso executable
blast2iso_path = lastools_path + "\\blast2iso.exe"

### check if executable exists
if os.path.exists(lastools_path) == False:
    gp.AddMessage("Cannot find blast2iso.exe at " + blast2iso_path)
    sys.exit(1)
else:
    gp.AddMessage("Found " + blast2iso_path + " ...")

### create the command string for blast2iso.exe
command = ['"' + blast2iso_path + '"']
...
### report output of blast2iso
gp.AddMessage(str(output))

### check return code
if returncode != 0:
    gp.AddMessage("Error. blast2iso failed.")
    sys.exit(1)

### report happy end
gp.AddMessage("Success. blast2iso done.")
