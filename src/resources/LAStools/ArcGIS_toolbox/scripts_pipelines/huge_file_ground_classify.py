...
#
# This LAStools pipeline ground-classifies very large
# LAS or LAZ files by operating with a tile-based
# multi-core pipeline. The input file is first tiled
# using lastile with the specified tile size and the
# specified buffer to avoid edge artifacts. All tiles
# are then ground classified using lasground marking
# points as ground (class 2) and non-gound (class 1).
# Finally the ground-classified tiles are rejoined back
...
gp.AddMessage("Starting huge_file_ground_classify ...")
...
command.append("-o")
command.append("temp_huge_ground_classify.laz")
...
### check return code
if returncode != 0:
    gp.AddMessage("Error. huge_file_ground_classify failed in lastile step.")
    sys.exit(1)
...
command.append("-i")
if empty_temp_dir != "#":
    command.append('"' + empty_temp_dir + "\\temp_huge_ground_classify*.laz" + '"')
else:
    command.append("temp_huge_ground_classify*.laz")
...
### check return code
if returncode != 0:
    gp.AddMessage("Error. huge_file_ground_classify failed in lasground step.")
    sys.exit(1)
...
command.append("-i")
if empty_temp_dir != "#":
    command.append('"' + empty_temp_dir + "\\temp_huge_ground_classify*_g.laz" + '"')
else:
    command.append("temp_huge_ground_classify*_g.laz")
...
### check return code
if returncode != 0:
    gp.AddMessage("Error. huge_file_ground_classify failed in clean-up step.")
    sys.exit(1)
...
### report happy end
gp.AddMessage("Success. huge_file_ground_classify done.")
