...
### check return code
if returncode != 0:
    gp.AddMessage("Error. huge_file_classify failed in lastile step.")
    sys.exit(1)
...
### check return code
if returncode != 0:
    gp.AddMessage("Error. huge_file_classify failed in lasground step.")
    sys.exit(1)
...
###################################################
### third step: compute heights (maybe drop points)
###################################################
...
### store height-computed tiles in compressed format
command.append("-olaz")
...
### check return code
if returncode != 0:
    gp.AddMessage("Error. huge_file_classify failed in lasheight step.")
    sys.exit(1)
...
###################################################
### fourth step: classify building and tree points
###################################################
...
### give classified tiles a meaningful appendix
command.append("-odix")
command.append("c")

### store classified tiles in compressed format
command.append("-olaz")
...
### check return code
if returncode != 0:
    gp.AddMessage("Error. huge_file_classify failed in lasclassify step.")
    sys.exit(1)

### report success
gp.AddMessage("lasclassify step done.")

###################################################
### fifth step: reverse the tiling
###################################################
...
### check return code
if returncode != 0:
    gp.AddMessage("Error. huge_file_classify failed in lastile (reverse) step.")
    sys.exit(1)
...
### check return code
if returncode != 0:
    gp.AddMessage("Error. huge_file_classify failed in clean-up step.")
    sys.exit(1)
...
### report happy end
gp.AddMessage("Success. huge_file_classify done.")
